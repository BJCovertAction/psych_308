...
Before we define our linear models for the purpose of inspecting homoscedasticity, we need to define our moderation  
variables
...
# Create Residuals plot for Slapstick Model
# Includes moderation but no categorical predictors references to no-preference
sck_resid_mod_fig = ggplot( slapstick_lm, aes(.fitted, .resid) )

sck_resid_mod_fig +
    geom_point(col = font_color) +
    geom_hline(yintercept=0, col="green3", linetype="dashed") +
    xlab("Fitted Time 2 Anxiety Levels (1 - 10") +
    ylab("Time 2 Anxiety Residuals (1 - 10)") +
    ggtitle("Slapstick: Main + Moderation Effects T2 Anxiety Residual vs. Fitted Plot - Ref'd to No-Preference") +
    my_gg_theme

# Slapstick Residuals Scaled by SD
sck_resid_mod_sd_fig = ggplot( slapstick_lm, aes(.fitted, (.resid / sd(clean_cent_dat$avg_stai_mid) )  ) )

sck_resid_mod_sd_fig +
    geom_point(col = font_color) +
    geom_hline(yintercept=0, col="green3", linetype="dashed") +
    xlab("Fitted Time 2 Anxiety Levels (1 - 10") +
    ylab(expression( "Residuals - Standardized (Multiples of " * sigma * ")") ) +
    ggtitle("Slapstick STD: Main + Moderation Effects T2 Anxiety Residual vs. Fitted Plot - Ref'd to No-Preference") +
    my_gg_theme


# Create Residuals plot for GlitterBomb Model
# Includes moderation AND categorical predictors referenced to Incongruent Humor Experience
gbm_resid_mod_fig = ggplot( glitterbomb_lm, aes(.fitted, .resid) )
...
gbm_resid_mod_fig +
    ...
    xlab("Fitted Time 2 Anxiety Levels (1 - 10") +
    ylab("Time 2 Anxiety Residuals (1 - 10)") +
    ggtitle("Glitterbomb: Main + Moderation Effects T2 Anxiety Residual vs. Fitted Plot - Ref'd to Incongruent") +
    my_gg_theme

# Glitterbomb Residuals Scaled by SD
gbm_resid_mod_fig = ggplot( glitterbomb_lm, aes(.fitted, (.resid / sd(clean_cent_dat_red$avg_stai_mid)) ) )
...
gbm_resid_mod_fig +
    ...
    xlab("Fitted Time 2 Anxiety Levels (1 - 10") +
    ylab(expression( "Residuals - Standardized (Multiples of " * sigma * ")") ) +
    ggtitle("Glitterbomb STD: Main + Moderation Effects T2 Anxiety Residual vs. Fitted Plot - Ref'd to Incongruent") +
    my_gg_theme 
...
cat( paste("  ", "---- Breusch-Pagan Slapstick ----", " ", sep = "\n") )
car::ncvTest(slapstick_lm)

cat( paste("  ", "---- Breusch-Pagan GlitterBomb ----", " ", sep = "\n") )
car::ncvTest(glitterbomb_lm)

```


## Multi-Variate Normality - Slapstick
...
ggplot(slapstick_lm,
    ...
    ggtitle("Slapstick Normal Q-Q of Main Effects Model") +
    my_gg_theme

# Henze-Zirkler test of multivariate normality
# Slapstick model uses non-reduced data. Need to test our analysis variables only (continuous)
HZ.test(clean_cent_dat[analysis_cont_vars] )

```

## Multi-Variate Normality - Glitterbomb

```{r multivar_norm_gb}

# Code below manually produces QQ Plot for better alignment with general plot styles
ggplot(glitterbomb_lm,
         aes(
               qqnorm( .stdresid,
                       xlab = "", ylab = "",
                       conf.level = 0.945,
...
# Henze-Zirkler test of multivariate normality
# Glitterbomb model uses reduced data. Need to test our analysis variables only (continuous)
HZ.test(clean_cent_dat_red[analysis_cont_vars] )
...
glitterbomb_def <- '
                     ...
                     avg_stai_before ~~ humor_pref_cont
                      ...
                      # Main Effects + Moderation + Cats for Congruence Refd to incongruence
                     avg_stai_mid ~ bef_mid*avg_stai_before + hum_mid*humor_pref_cont + cong_mid*cong_dc + mod_cng_hum*cong_x_hp 
...
# Bootstrap!
# Must use WLS if we do have ordered vars.
glitterbomb_fit <- lavaan::sem(  glitterbomb_def,
                                 data = clean_cent_dat_red,
                                 se = "bootstrap",
                                 estimator = "ML",
                                 bootstrap = 10000,
...
# Scrollable table in HTML output
standardizedSolution(glitterbomb_fit, type = "std.all")

# Additional modelfit output with CIs that's a bit more user friendly to text editors
parameterEstimates(glitterbomb_fit, standardized = TRUE, boot.ci.type = "perc", level=0.95)
