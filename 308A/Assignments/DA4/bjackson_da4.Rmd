...
Specify, Criteria, Estimate, Conclude:  
 ...
 3. **Estimate:**  Given the test conducted in the R-code above, we found the  
    ...
    generated in text, on the fly, using the embedded r/tex-code shown afer the  
    printout:
    
    $\bar{x}_{diff}\ =\ $ `r rq1_test_df$'md[stud]'`    
    $ESE_{diff}\ =\ $ `r rq1_test_df$'sed[stud]'`  
    $t_{calc}\ =\ $ `r rq1_test_df$'stat[stud]'`  
    `r if( rq1_test_df$'p[stud]' >= 0.000001 ){ paste("p = ", str_sub(sprintf("$.6f", rq1_test_df$'p[stud]'), -7 ) ) }else{ "p < .000001" }`  
...
(midterm, $\bar{x}\ =\ 72.40,\ SD\ =\ 10.60$), t(99) = 7.57, p < .001, $\eta^2$ = 0.37.  
...
See attached hand-drawn image below:
